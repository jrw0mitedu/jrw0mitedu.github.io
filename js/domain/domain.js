// the domain carousel 
// ----------------------------------

var slides = [];
slides.push({
    title       : 'Lora Design',
    subtitle    : 'Design Research',
    description : 'Augmented Reality, the Internet of Things (IOT), Blockchain, and Educational Technology (EdTech).',
    image       : 'data:image/gif;base64,R0lGODlhAQABAIAAAHd3dwAAACH5BAAAAAAALAAAAAABAAEAAAICRAEAOw==',
    active    : true        
});
slides.push({
    title       : 'Simulator',
    subtitle    : 'Global Infrastructure Data Simulator',
    description : 'The GDC Simulator provides decision makers with a tool that allows understanding and predicting the consequences of the deployment of new network topologies, hardware conﬁgurations or software applications in a global data infrastructure, without affecting the service. The simulator was validated on the data infrastructure of a Fortune 100 company.',
    image       : 'data:image/gif;base64,R0lGODlhAQABAIAAAHd3dwAAACH5BAAAAAAALAAAAAABAAEAAAICRAEAOw=='
});
slides.push({
    title       : 'Hacking Education',
    subtitle    : 'EdTech Next Gen',
    description : 'With a long tradition of Educational Technology (EdTech) research, the GDC is investigating student data analytics/ML, the 360 degree view, new teaching platforms, feedback acceleration, connected intelligence, microcredentials, as well as scalable low-cost bootcamps. ',
    image       : 'data:image/gif;base64,R0lGODlhAQABAIAAAHd3dwAAACH5BAAAAAAALAAAAAABAAEAAAICRAEAOw=='
});
slides.push({
    title       : 'Security',
    subtitle    : 'Cyber Physical Systems',
    description : 'The GDC has ongoing cybersecurity projects in Risk Modeling, Situational Awareness, Behavior Analysis, Negative Authentication, and Critical Infrastructure protection.',
    image       : 'data:image/gif;base64,R0lGODlhAQABAIAAAHd3dwAAACH5BAAAAAAALAAAAAABAAEAAAICRAEAOw=='
});

// the domain features 
// ----------------------------------

var features = [];
features.push({
    title       : 'Hello Title',
    description : 'This is a wider card with supporting text below as a natural lead-in to additional content. This content is a little bit longer.',
    image       : 'img/stub.png'
});
features.push({
    title       : 'Hello Title',    
    description : 'This is a wider card with supporting text below as a natural lead-in to additional content. This content is a little bit longer.',
    image       : 'img/stub.png'
});
features.push({
    title       : 'Hello Title',    
    description : 'This is a wider card with supporting text below as a natural lead-in to additional content. This content is a little bit longer.',
    image       : 'img/stub.png'
});
features.push({
    title       : 'Hello Title',    
    description : 'This is a wider card with supporting text below as a natural lead-in to additional content. This content is a little bit longer.',
    image       : 'img/stub.png'
});

// the domain thumbnails 
// ----------------------------------

var thumbnails = [];
thumbnails.push({
    description : 'This is a wider card with supporting text below as a natural lead-in to additional content. This content is a little bit longer.',
    image       : 'img/thumb.png'
});
thumbnails.push({
    description : 'This is a wider card with supporting text below as a natural lead-in to additional content. This content is a little bit longer.',
    image       : 'img/thumb.png'
});
thumbnails.push({
    description : 'This is a wider card with supporting text below as a natural lead-in to additional content. This content is a little bit longer.',
    image       : 'img/thumb.png'
});
thumbnails.push({
    description : 'This is a wider card with supporting text below as a natural lead-in to additional content. This content is a little bit longer.',
    image       : 'img/thumb.png'
});
